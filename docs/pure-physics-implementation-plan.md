...
1. **dissipationLoss_H の符号条件を修正する。** dissipationLoss_N ≥ 0 は定理として要求するが、dissipationLoss_H は符号保証しない。実測値をそのまま記帳する。
1. **tick内ステップ順序を固定する。** 基本順序は以下とする:保存部 → 散逸 → 駆動 → 媒質履歴 → 観測
1. **step orderをsolver settingsとしてexportに含める。** 将来順序を変更した場合、物理差分なのか順序差分なのかを区別できるようにする。
...
1. 場へ触れる入力経路は J(x,t) のみ。
1. 観測は readonly。観測値は力学へ戻さない。
1. clamp / maxDelta / amplitudeClamp / max(ν,0) を入れない。
1. すべての差分は帳簿に名前付きで記録する。
1. 既存coreを置き換えず、src/pure/ として並走させる。
1. 実装はPRごとに一層ずつ進め、各PRで対応する検証ハーネスを通す。
1. 物理の破れ・NaN・発散・帳簿不成立は隠さず、実験の事実として記録する。
1. 観測者は「こう見える」を報告できるが、確定しない。
1. Aeterna本体の実出力と観測者の解釈を混同しない。
...
1. **Conservative block** — 非線形位相 half step / 線形 Cayley/CN step / 非線形位相 half step
1. **Dissipation step** — ψ ← ψ · exp(−νdt)
1. **Drive step** — ψ ← ψ + J(x,t)dt
1. **Medium history step** — ν(x) を厳密積分で更新
1. **Observe step** — readonly snapshotから観測値を導出

この順序は、実験条件の一部である。順序を変えれば、O(dt)レベルで結果が変わりうる。
...
これはCFL条件では救えない。シュレディンガー型の分散項に対して、前進オイラーは無条件不安定である。

したがって、**クランプ撤廃と積分器置換は不可分である。** クランプは「安全策」ではなく、壊れた積分器が生む増幅を隠す応急処置になりうる。
...
```
ψ_next = (I − iαdt L/2)⁻¹ (I + iαdt L/2) ψ
```
...
トーラス曲面では、セル面積 dA や計量係数が場所によって変わる。したがって、単純な差分ラプラシアンでは保存構造が壊れる可能性がある。
...
```
H = α <ψ, −Lψ>_dA + (g/2) Σ |ψ|⁴ dA
```

必要に応じて係数や符号は実装する L の符号規約に合わせる。重要なのは、Hがstepと同一の離散演算子から導かれることである。
...
曲面計量でCayley / Crank–Nicolsonを使う場合、線形方程式の求解が必要になる。反復解法を使う場合、その許容誤差は場の挙動に影響するため、隠してはならない。

以下は物理定数ではないが、runtimeの数値解法設定として明示する。

|記号                       |意味                              |種別         |
|-------------------------|--------------------------------|-----------|
|linearSolverTolerance    |Cayley/CN線形ソルバーの収束許容誤差          |数値解法       |
|linearSolverMaxIterations|Cayley/CN線形ソルバーの最大反復回数          |数値解法       |
|linearSolverKind         |direct / iterative / spectral など|数値解法       |
|solverStepOrder          |tick内ステップ順序                     |数値解法 / 実験条件|

これらは「欲しい振る舞いを作るための調整定数」ではない。ただしruntime結果に影響するため、seed・dt・Nと同じく、実験ログとexportに必ず含める。
...
|項                |意味                   |符号            |
|-----------------|---------------------|--------------|
|driveWork_N      |駆動JがNに与えた符号付き仕事      |正/負どちらもありうる   |
|driveWork_H      |駆動JがHに与えた符号付き仕事      |正/負どちらもありうる   |
|dissipationLoss_N|散逸νによりNから失われた量       |常に非負          |
|dissipationLoss_H|散逸ステップ前後のH減少量としての実測差分|符号保証なし        |
|numericalDrift_H |保存ブロックの数値積分誤差        |物理ではなく計算上の観測項 |
|residual_N/H     |上記すべてを記帳した後の残差       |実装バグ・帳簿不成立の検出用|

### 重要修正: H側の散逸は符号保証しない

dissipationLoss_N ≥ 0 は定理として要求する。各セルの |ψ| が指数減衰するため、Nは必ず減る。
...
したがって、H側では散逸ステップ前後の実測差分をそのまま記帳する。符号を無理に正へ揃えない。
...
**目的:** PURE PHYSICSの憲法をdocsとして固定する。
...
**目的:** 散逸・駆動・媒質履歴を入れる前に、保存部だけを証明する。
...
**目的:** weak plasticityを観測カテゴリではなく、物理的な媒質履歴として実装する。
...
厳密積分 — tick内で |ψ|² を凍結すれば、νについて線形ODEになる:
...
**目的:** 純粋物理コアに観測器を付ける。ただし観測は力学に戻さない。
...
1. 既存coreを変更していないか
1. pure coreがlegacy/organism層に依存していないか
1. 観測値が力学に戻っていないか
1. 場への入力経路はJだけか
1. 差分は帳簿に名前付きで記録されているか
1. residualを隠していないか
1. numericalDrift_Hを濫用していないか
1. clamp系が入っていないか
1. 追加パラメータは物理・幾何・離散化・数値解法のどれかとして説明できるか
1. 「欲しい振る舞い」の直接記述になっていないか
1. 観測者の表現とAeterna actual outputを混同していないか
1. 線形部に前進オイラーが入っていないか
1. 離散ラプラシアンLは自己随伴か
1. invariantsとstepは同じLを使っているか
1. solverStepOrderは固定され、exportされているか
1. dissipationLoss_NとdissipationLoss_Hの符号条件を混同していないか
