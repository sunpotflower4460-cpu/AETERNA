...
| K7追加・探索的follow-up（持続優位性の機構分離） | E（探索的分析、新規の確証的閾値判定なし） | `src/tests/pure/` 3テスト追加（計266）。条件2を構成する2自由度（媒質履歴・χ）を分離した2条件を実行: 媒質履歴のみは条件1（最小構成）と完全一致（優位性ゼロ、5 seed全て）、χのみは条件2と1tick以内で一致（優位性のほぼ全てを再現、5 seed全て）。**結論**: 30倍の持続優位性はχにほぼ完全に起因し、媒質履歴の寄与は無視できる。機構の仮説（χの局所的対称性の破れが欠陥核形成に必要）は記録するが証明はしていない |
