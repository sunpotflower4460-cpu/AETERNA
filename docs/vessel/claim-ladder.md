...
| K6（生命的閉路観測） | 測定器はC3、実測知見はE（要注意フラグ付き） | `src/tests/pure/` 27 テスト追加（計243）で測定器自体を検証: 条件A/Bの決定性・较正の妥当性（線形近似の範囲内）・判定統計量の算術（合成データで検証）を確認。**実測**（1回のみ、事前登録どおり）: 境界セル局所密度は条件A・B間で統計的に区別できた（`\|mean\|/sem≈16.7`、20 seed全てで条件Bが条件Aの約1.5〜2.5倍という一貫した方向）。大域コヒーレンスは区別できなかった（`≈1.08`）。L2判定は`tau_min`が観測窓を超える設計上の理由で判定不能（実測後に発覚、事後調整はしていない）。**要注意（第8監査）**: 観測された差は「状態依存・有界なユニタリ回転」対「状態非依存・無界な加法」という数学的構造の違いだけで説明できる可能性があり、この実測単体では自他弁別の確証にならない。「AETERNAが自他を区別した」とは言わない（`VESSEL_CHARTER.md` §5） |
| K7（天井の地図、docs-only部分） | C1 | `docs/vessel/white-ceilings.md` に3列の地図表を作成。新規実測なし。K2〜K6を通じてemergence levelの系統的な実測が0件だったことを明記 |
| K7追加（自然発展下でのL2実測） | 測定器はC2、実測知見はN（誠実な負の結果） | `src/tests/pure/` 12テスト追加（計263）。**実測**（1回のみ、事前登録どおり）: L2は条件1（ν均一・χなし）・条件2（媒質履歴+χ）とも5 seed全てで未達（`tau_min=500`tickに対し実持続は条件1で1tick、条件2で34〜39tick）。渦候補は過渡的に現れて最終tickには消える。**部分的な発見**: 条件2は条件1よりおよそ30倍長く渦候補を保持する、5 seed全てに一貫した傾向。K2 PR5（L3見込み）・K3（L4候補）の両仮説は、この実測単体では支持も棄却もされず「未確定」のまま。`tau_min`の選定自体が力学の実際のスケールに対し過大だった可能性を明記し、事後の閾値調整はしていない |
